...
#include "LerosGenDAGISel.inc" // TableGen output goes here
...
bool LerosDAGToDAGISel::isOrEquivalentToAdd(const SDNode *N) const {
  ...
  // Use KnownBits analysis - CurDAG is accessible as a member!
  // We use 'CurDAG' which is a member of SelectionDAGISel
  KnownBits KnownOp0 = CurDAG->computeKnownBits(Op0);
  KnownBits KnownOp1 = CurDAG->computeKnownBits(Op1);
  ...
  // Note: The original dump used '&DAG', but since we are in a member function,
  // we can use 'CurDAG'.
  LLVM_DEBUG(dbgs() << "isOrEquivalentToAdd: KnownBits inconclusive for node: "; N->dump(CurDAG); dbgs() << "\n");
  return false;
}
} // namespace llvm
